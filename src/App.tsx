  ...
  return (
    <div className="min-h-screen bg-background text-on-surface flex flex-col md:flex-row font-sans" dir="rtl">
        ...
        <div className="flex-1 py-6 flex flex-col justify-between">
          <nav className="space-y-1.5 px-3">
            {([
              ...
              { tab: 'inventory', icon: <Boxes className="w-5 h-5" />, label: 'إدارة المخزون' },
              { tab: 'sales', icon: <Receipt className="w-5 h-5" />, label: 'سجل الفواتير' },
      ...
      <nav className="fixed bottom-0 inset-x-0 bg-white border-t border-outline-variant md:hidden z-50 shadow-lg flex flex-row justify-around items-center h-20 pb-safe px-2 print:hidden">
        {([
          { tab: 'dashboard', icon: <LayoutDashboard className="w-5 h-5 mb-1" />, label: 'الرئيسية' },
          { tab: 'inventory', icon: <Boxes className="w-5 h-5 mb-1" />, label: 'المخزن' },
          { tab: 'sales', icon: <Receipt className="w-5 h-5 mb-1" />, label: 'المبيعات' },
